...
namespace Detetive.SEC
{
    public sealed class DetetiveMembershipProvider : MembershipProvider
    {
        private string eventSource      = "DetetiveMembershipProvider";
        ...
        public override void Initialize(string name, NameValueCollection config)
        {
            ...
            if (name == null || name.Length == 0)
                name = "DetetiveMembershipProvider";

            //assume a descrição padrão, se não foi passada
            if (String.IsNullOrEmpty(config["description"]))
            {
                config.Remove("description");
                config.Add("description", "Detetive MS SQL Server Membership Provider");
            }
